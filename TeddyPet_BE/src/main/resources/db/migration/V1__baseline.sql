...
-- Phần đầu: Hibernate baseline gốc (Thu Feb 26 22:26:26 +07 2026), sau đó là các ALTER/CREATE từ V2..V82
    ...
    create table if not exists booking_pets (
        ...
        pet_condition_notes TEXT,
            confirmed_pet_type      VARCHAR(100),
    confirmed_weight        DECIMAL(6, 2),
...
--     booking_check_in_date  TIMESTAMP WITHOUT TIME ZONE,
--     booking_check_out_date TIMESTAMP WITHOUT TIME ZONE,
--     booking_date_from      date,
...
-- Idempotent: DB cũ có thể chưa có bảng contracts (không thể drop full DB production)
DO $$
BEGIN
  IF NOT EXISTS (
  ...
  ELSE
    ALTER TABLE contracts ADD COLUMN IF NOT EXISTS contract_type VARCHAR(20) DEFAULT 'FULL_TIME';
  END IF;
END $$;
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = current_schema() AND table_name = 'contracts' AND column_name = 'contract_type'
  ) THEN
    EXECUTE 'COMMENT ON COLUMN contracts.contract_type IS ' ||
      quote_literal('Loại hợp đồng: FULL_TIME, PART_TIME');
  END IF;
END $$;
...
ALTER TABLE work_shift_registrations
    ADD CONSTRAINT work_shift_registrations_status_check
        CHECK (status IN ('PENDING', 'APPROVED', 'REJECTED', 'ON_LEAVE')) NOT VALID;

-- ========== V3__add_pending_leave_registration_status.sql ==========
...
-- ========== V2__add_leave_decision_to_registrations.sql ==========
...
-- ========== V4__remove_hire_date_from_staff_profiles.sql ==========
...
-- ========== V5__shift_role_config_and_registration_role_work_type.sql ==========
...
-- ========== V6__ensure_service_room_types_table.sql ==========
...
-- ========== V7__rename_booking_pet_services_check_dates_to_estimated.sql ==========
...
-- ========== V8__ensure_staff_fixed_schedules_table.sql ==========
...
-- ========== V9__add_room_type_id_to_service_pricing.sql ==========
...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='bookings' AND column_name='booking_start_date')
       AND NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='bookings' AND column_name='booking_check_in_date') THEN
        ALTER TABLE bookings RENAME COLUMN booking_start_date TO booking_check_in_date;
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='bookings' AND column_name='booking_end_date')
       AND NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='bookings' AND column_name='booking_check_out_date') THEN
...
create index if not exists idx_order_refund_admin_evidence_refund_id on order_refund_admin_evidence (refund_id);
