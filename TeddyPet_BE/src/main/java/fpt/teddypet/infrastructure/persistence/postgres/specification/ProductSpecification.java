...
public class ProductSpecification {
    ...
    public static Specification<Product> buildBrandFilterSpecification(Long brandId) {
        ...
        return (root, query, criteriaBuilder) -> 
            criteriaBuilder.equal(root.get(Product_.brand).get(ProductBrand_.id), brandId);
    }
    ...
    public static Specification<Product> buildPetTypeFilterSpecification(List<PetTypeEnum> petTypes) {
        ...
        return (root, query, criteriaBuilder) -> {
            ...
            // For ElementCollection, we need to check if any petType in the collection matches
    ...
    public static Specification<Product> buildStatusFilterSpecification(ProductStatusEnum status) {
        ...
        return (root, query, criteriaBuilder) -> 
            criteriaBuilder.equal(root.get(Product_.status), status);
    }
    
    public static Specification<Product> buildStockFilterSpecification(String stockStatus, Integer threshold, Boolean includeDeleted) {
        ...
        return (root, query, criteriaBuilder) -> {
            ...
            List<Predicate> variantPredicates = buildStockStatusPredicates(
                variantRoot, criteriaBuilder, stockStatus.trim().toUpperCase(), threshold);
    ...
    private static List<Predicate> buildStockStatusPredicates(
            Root<ProductVariant> variantRoot,
            CriteriaBuilder criteriaBuilder,
            String stockStatusUpper,
            Integer threshold) {
        List<Predicate> predicates = new ArrayList<>();
        
        if ("OUT_OF_STOCK".equals(stockStatusUpper)) {
            predicates.add(
                criteriaBuilder.equal(variantRoot.get(ProductVariant_.stockQuantity).get(StockQuantity_.value), 0)
            );
        } else if ("LOW_STOCK".equals(stockStatusUpper)) {
            int stockThresholdValue = (threshold != null && threshold > 0) ? threshold : 10;
            predicates.add(
                criteriaBuilder.lessThan(variantRoot.get(ProductVariant_.stockQuantity).get(StockQuantity_.value), stockThresholdValue)
            );
        }
