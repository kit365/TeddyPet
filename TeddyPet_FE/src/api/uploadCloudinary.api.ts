...
/** Cache cấu hình Cloudinary (dùng chung với admin/product). */
...
/**
 * Upload ảnh trực tiếp lên Cloudinary (giống trang staff/product).
 * Dùng upload_preset unsigned, folder tùy chọn.
 */
export const uploadImagesToCloudinary = async (
    files: File[],
    folder: string = "teddypet"
): Promise<string[]> => {
    try {
        ...
        const errorMessage =
            error.response?.data?.error?.message || "Lỗi khi tải ảnh lên server.";
