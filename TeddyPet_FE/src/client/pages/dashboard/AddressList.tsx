        ...
        <DashboardLayout pageTitle="Danh sách địa chỉ" breadcrumbs={breadcrumbs}>
            <div className="flex justify-between items-end border-b border-slate-100 pb-8 mb-10">
                <div>
                    ...
                    <p className="text-[0.75rem] text-slate-400 font-medium mt-1 uppercase tracking-widest">Nơi nhận yêu thương từ TeddyPet</p>
                </div>
                <Link to="/dashboard/address/create" className="flex items-center gap-2 bg-client-primary text-white px-8 py-3.5 rounded-2xl font-black text-[0.6875rem] uppercase tracking-widest hover:bg-client-secondary transition-all shadow-xl shadow-client-primary/20">
            ...
            {loading ? (
                <div className="py-20 flex flex-col items-center gap-4">
                    ...
                    <p className="text-[1rem] font-bold text-slate-300">Đang tải danh sách...</p>
                </div>
            ) : (
                <div className="grid grid-cols-2 gap-8">
                    {addresses.map((item) => (
                        <div key={item.id} className="group relative bg-white border border-slate-100 rounded-[1.5625rem] p-8 transition-all hover:shadow-2xl hover:shadow-client-primary/5 hover:-translate-y-1">
                            {item.isDefault && (
                                <div className="absolute top-6 right-6 bg-emerald-50 text-emerald-600 text-[0.6875rem] font-black px-4 py-1.5 rounded-full uppercase tracking-widest border border-emerald-100">
                                    Mặc định
                                </div>
                            )}
                            <div className="flex gap-6">
                                ...
                                <div className="flex-1 space-y-4">
                                    <div>
                                        <div className="flex items-center gap-3 text-[1.125rem] text-slate-800 font-bold">
                                            <UserIcon className="w-6 h-6 text-client-primary" />
                                            {item.fullName}
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-3 text-[0.9375rem] text-slate-500 font-medium font-sans">
                                        <Phone className="w-6 h-6 text-slate-300" />
                                        {item.phone}
                                    </div>
                                    <div className="flex items-start gap-3 text-[0.9375rem] text-slate-500 font-medium leading-relaxed font-sans">
                                    ...
                                    <div className="pt-4 flex items-center gap-6 border-t border-slate-50">
                                        <Link to={`/dashboard/address/edit/${item.id}`} className="text-[0.75rem] font-black text-client-primary uppercase tracking-widest hover:text-client-secondary transition-colors">Chỉnh sửa</Link>
                                        <button
                                            onClick={() => handleDelete(item.id)}
                                            className="text-[0.75rem] font-black text-rose-500 uppercase tracking-widest hover:text-rose-700 transition-colors"
                                        >
                                            Xóa bỏ
                                        </button>
                                    </div>
                    ...
                    {addresses.length === 0 && !loading && (
                        <div className="col-span-2 py-20 flex flex-col items-center gap-6 opacity-40">
                            ...
                            <p className="text-[1.125rem] font-bold text-slate-400">Bạn chưa có địa chỉ nào lưu lại.</p>
                        </div>
                    )}
                </div>
            )}
