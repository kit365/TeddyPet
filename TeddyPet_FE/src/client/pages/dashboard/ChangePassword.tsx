        ...
        <DashboardLayout pageTitle="Bảo mật tài khoản" breadcrumbs={breadcrumbs}>
            <div className="flex items-center justify-between mb-12">
                <div className="flex items-center gap-6">
                    <div className="w-20 h-20 bg-red-50 rounded-[1.25rem] flex items-center justify-center text-client-primary shadow-inner">
                        <ShieldCheck className="w-10 h-10" />
                    </div>
                    <div>
                        <h3 className="text-[1.625rem] font-black text-slate-800 tracking-tight leading-tight">
                            Thay đổi mật khẩu
                        </h3>
                ...
                <div className="hidden lg:block">
                    <div className="bg-emerald-50 px-6 py-3 rounded-2xl border border-emerald-100 flex items-center gap-3">
                        ...
                        <span className="text-[0.75rem] font-black text-emerald-700 uppercase tracking-widest">Tài khoản an toàn</span>
                    </div>
                </div>
            </div>
                ...
                <div className="flex justify-between relative z-10">
                    {steps.map((s, idx) => (
                        <div key={idx} className="flex flex-col items-center">
                            ...
                            <span className={`mt-4 text-[0.75rem] font-black uppercase tracking-widest ${idx <= step ? 'text-client-primary' : 'text-slate-300'}`}>
                                {s.label}
                            </span>
                        </div>
                    ))}
            ...
            <div className="max-w-[700px] mx-auto">
                <div className="bg-white rounded-[1.875rem] border border-slate-100 shadow-2xl shadow-indigo-100/20 overflow-hidden min-h-[400px] flex flex-col">
                    <div className="p-12 flex-1 flex flex-col justify-center">
                        {step === 0 && (
                            <div className="text-center animate-fadeIn space-y-8">
                                ...
                                <div>
                                    <h4 className="text-[1.375rem] font-black text-slate-800 mb-2">Gửi mã xác thực</h4>
                                    <p className="text-[0.9375rem] text-slate-400 font-medium">Chúng tôi sẽ gửi một mã OTP gồm 6 chữ số đến email đã đăng ký của bạn.</p>
                                </div>
                                <button
                                    onClick={handleSendOtp}
                                    disabled={isSendingOtp || cooldown > 0}
                                    className="w-full h-[65px] bg-client-primary hover:bg-slate-800 text-white rounded-2xl font-black text-[1rem] transition-all flex items-center justify-center gap-3 shadow-xl shadow-red-100 disabled:opacity-50"
                                >
                        ...
                        {step === 1 && (
                            <div className="animate-fadeIn space-y-6">
                                <div className="flex items-center gap-4 mb-4">
                                    ...
                                    <h4 className="text-[1.25rem] font-black text-slate-800">Nhập mã OTP</h4>
                                </div>
                                <div className="space-y-4">
                                    <p className="text-[0.875rem] text-slate-400 font-medium italic">Vui lòng kiểm tra email và nhập mã xác thực gồm 6 chữ số.</p>
                                    <input
                                        type="text"
                                        maxLength={6}
                                        value={otpCode}
                                        onChange={(e) => setOtpCode(e.target.value.replace(/[^0-9]/g, ''))}
                                        className="w-full bg-slate-50 border-2 border-slate-50 rounded-2xl px-8 py-6 text-[1.875rem] font-black text-center tracking-[0.9375rem] focus:outline-none focus:border-client-primary focus:bg-white transition-all placeholder:text-slate-200"
                                        placeholder="000000"
                                    />
                                    <button
                                        onClick={handleNextStep}
                                        disabled={isLoading}
                                        className="w-full h-[65px] bg-client-primary hover:bg-slate-800 text-white rounded-2xl font-black text-[1rem] transition-all flex items-center justify-center gap-3 shadow-xl disabled:bg-slate-300"
                                    >
                                        {isLoading ? (
                        ...
                        {step === 2 && (
                            <div className="animate-fadeIn space-y-6">
                                <div className="flex items-center gap-4 mb-4">
                                    <button onClick={handleBackStep} className="p-3 hover:bg-slate-50 rounded-xl transition-colors text-slate-400"><ArrowLeft /></button>
                                    <h4 className="text-[1.25rem] font-black text-slate-800">Mật khẩu hiện tại</h4>
                                </div>
                                <div className="space-y-4">
                                    <p className="text-[0.875rem] text-slate-400 font-medium">Để đảm bảo tính chính danh, vui lòng xác nhận mật khẩu đang sử dụng.</p>
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            className="w-full bg-slate-50 border-2 border-slate-50 rounded-2xl pl-16 pr-8 py-6 text-[1rem] font-bold focus:outline-none focus:border-client-primary focus:bg-white transition-all"
                        ...
                        {step === 3 && (
                            <div className="animate-fadeIn space-y-6">
                                <div className="flex items-center gap-4 mb-4">
                                    <button onClick={handleBackStep} className="p-3 hover:bg-slate-50 rounded-xl transition-colors text-slate-400"><ArrowLeft /></button>
                                    <h4 className="text-[1.25rem] font-black text-slate-800">Thiết lập mật khẩu mới</h4>
                                </div>
                                <form onSubmit={onSubmit} className="space-y-5">
                                    <div className="space-y-2">
                                        <label className="text-[0.6875rem] font-black text-slate-400 uppercase tracking-widest pl-2">Mật khẩu mới</label>
                                        <div className="relative">
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-[0.6875rem] font-black text-slate-400 uppercase tracking-widest pl-2">Xác nhận mật khẩu</label>
                                        <div className="relative">
                                            <Lock className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400 w-6 h-6" />
                                            <input
                                                type="password"
                                    ...
                                    <button
                                        type="submit"
                                        disabled={isLoading}
                                        className="w-full h-[70px] bg-slate-900 hover:bg-client-primary text-white rounded-2xl font-black text-[1rem] transition-all flex items-center justify-center gap-3 shadow-2xl disabled:bg-slate-300"
                                    >
                                        {isLoading ? (
                                            <div className="w-6 h-6 border-4 border-white/20 border-t-white rounded-full animate-spin"></div>
                                        ) : (
